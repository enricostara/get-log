...
var fileStreamName = getFileStreamName();
var fileStream = fileStreamName && require('fs').createWriteStream(fileStreamName, {autoClose: false});
...
function getFileStreamName() {
    ...
    if (filePrefix) {
        ...
        var ts = now.getFullYear() + month + day;
        ts += '-' + now.getHours() + now.getMinutes();
        ts += '-' + now.getMilliseconds();
...
// Enable logger `namespace` , include namespaces separated by comma and use of wildcards
exports.enable = function (namespace) {
    debug.enable(namespace);
};

exports.logFileName = fileStreamName;
